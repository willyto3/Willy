...
public class SchoolGradingSystem extends GradingSystem {
    ...
    public static void loadData(String nombre1, String nota1, String materia1, String genero1) {
        ...
        float nombre = Float.parseFloat(nombre1);
        float genero = Float.parseFloat(nota1);
        float materia = Float.parseFloat(materia1);
        float nota = Float.parseFloat(genero1);

        String sql = "INSERT INTO Datos (nombre, materia, genero, nota) VALUES (?,?,?,?)";

        try (Connection conn = App.conexion(); // conexión a SQL
                PreparedStatement st = conn.prepareStatement(sql)) {
            st.setObject(1, nombre);
            st.setObject(2, materia);
            st.setObject(3, genero);
            st.setObject(4, nota);
            st.executeUpdate();
            ...
            estudiante = new SchoolGradingSystem(nombre, genero, materia, nota);
